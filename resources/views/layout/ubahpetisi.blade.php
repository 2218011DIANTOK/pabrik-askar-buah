   ...
   onclick="event.preventDefault();
    ...
    <div class="main-content">
        ...
        <div class="tabel-wrapper">
            ...
            <form action="/updatedata2/{{$petisi->id_petisi}}" method="POST">
                    @csrf
                    <div class="form-group">
                    <select name="id_gagasan">
                                    <option disabled selected>Pilih</option>
                                    @foreach($gagasan as $terminal)
                                    <option value="{{ $terminal->id_gagasan }}" {{ $petisi->id_gagasan == $terminal->id_gagasan ? 'selected' : '' }}>{{ $terminal->judul_gagasan }}</option>
                                    @endforeach
                                </select>
</div>
                    <div class="form-group">
                        <label for="judul_gagasan">Judul Gagasan</label>
                        <input type="text" id="judul_gagasan" name="judul_gagasan" value="{{$petisi->judul_gagasan}}" />
                    </div>
                    <div class="form-group">
                        <label for="narasi_gagasan">Narasi Gagasan</label>
                        <textarea id="narasi_gagasan" name="narasi_gagasan" rows="4">{{$petisi->narasi_gagasan}}</textarea>
                    </div>
                    <div class="form-group">
                    </div>
